...
$tableCheck = $mysqli->query("SHOW TABLES LIKE 'certificate'");
if (!$tableCheck) {
    http_response_code(500);
...
if ($tableCheck->num_rows === 0) {
    // No certificate table yet
    $mysqli->close();
    echo json_encode(['status' => 'ok', 'certificates' => []]);
    exit;
}

$sql = "SELECT id, consultation_id, patient_id, cert_type, rest_period, start_date, end_date, notes, doctor_name, created_at, updated_at
        FROM `certificate`";
...
while ($row = $result->fetch_assoc()) {
    $certificates[] = [
        ...
        'restPeriod' => $row['rest_period'] !== null ? (int)$row['rest_period'] : null,
...
echo json_encode(['status' => 'ok', 'certificates' => $certificates]);
